...
class GitHubArchiveAnalyzer:
            ...
            with gzip.GzipFile(fileobj=BytesIO(response.content)) as f:
                for line in f:
                    try:
                        ...
                        if 'type' in record and record['type'] in ['IssueCommentEvent', 'PushEvent', 'IssuesEvent', 'ReleaseEvent']:
                            # Check if the record has all required fields based on its type
                            is_valid = True
                            
                            # Common required fields for all events
                            if not all(key in record for key in ['id', 'type', 'created_at', 'actor']):
                                is_valid = False
                            
                            # For IssueCommentEvent, ensure we have comment data
                            if is_valid and record['type'] == 'IssueCommentEvent':
                                if not ('payload' in record and 
                                    'comment' in record['payload'] and 
                                    'body' in record['payload']['comment']):
                                    is_valid = False
                            
                            # For IssuesEvent, ensure we have issue number and action
                            if is_valid and record['type'] == 'IssuesEvent':
                                if not ('payload' in record and 
                                    'issue' in record['payload'] and 
                                    'number' in record['payload']['issue'] and
                                    'action' in record['payload']):
                                    is_valid = False
                            
                            # For PushEvent, ensure we have repository info
                            if is_valid and record['type'] == 'PushEvent':
                                if not ('repo' in record and 'name' in record['repo']):
                                    is_valid = False
                            
                            # Add the record if it passes all checks
                            if is_valid:
                                # Ensure standardized access to needed fields
                                if record['type'] == 'IssueCommentEvent':
                                    # Add a flattened field for the comment body
                                    record['comment.body'] = record['payload']['comment']['body']
    ...
    def process_toxicity(self, rater, batch_size=32):
        ...
        if os.path.exists(toxicity_scores_path):
            ...
            # print(comment_events.head())

            # print(comment_events.columns)
            # print("payload:")
            # print(comment_events['payload'].head())
            
            # Example: Accessing the "body" key from the payload column in the DataFrame
            # comment_events['payload'].apply(lambda x: x['comment']['body'] if 'comment' in x else None)
    
            # def get_comment_body(payload):
            #     try:

            #         print(payload)

            #         print("ac")
            #         # print(payload.apply(lambda x: x['action'] if isinstance(x, dict) else None))ċ
            #         # print(payload['action'])
            #         # print(payload.apply(lambda x: json.loads(x.replace("'", '"'))['action'] if isinstance(x, str) else None))
                   
            #         # # Alternative approach if json.loads() doesn't work
            #         # parsed_payload = payload.apply(lambda x: ast.literal_eval(x))
            #         # print(parsed_payload.apply(lambda x: x['action']))



            #         # print("comment")
            #         # print(payload.apply(lambda x: x['body'] if isinstance(x, dict) else None).head())
            ...
            # comment_events['comment_body'] = get_comment_body(comment_events['payload'])


            # x = pd.json_normalize(comment_events['payload'])
            # print('vvvvv')
            # print(x.columns)

            # print(comment_events['payload'].head())
            # Assuming comment_events is your dataframe
            # flattened = pd.json_normalize(comment_events.to_dict(orient='records'), sep='.')

            # Display the column names which represent the possible paths
            # print(flattened.columns)

            # print(comment_events['payload']['body'].head())
            # 
            # print(comment_events['payload'].apply(lambda x: x.get('body', None) if isinstance(x, dict) else None).head())
            # print('^^^^^')
            # Check if the required column exists
            #if 'payload.action.comment.body' in comment_events.columns:
            if 'payload' in comment_events.columns:
                print("YESSSSSIRRRRRRRRRR")
                # comments = comment_events['comment.body'].fillna('').tolist()   it gives error
                comments = comment_events['payload'].apply(lambda p: p.get('action', {}).get('comment', {}).get('body', '') if isinstance(p, dict) else '').tolist()
                ...
                logger.warning("Column 'comment.body' not found. Skipping toxicity processing.")
                self.ddf = self.ddf.assign(toxicity_score=0.0)
                self.ddf = self.ddf.assign(is_toxic=False)
    ...
    def generate_specific_csv_files(self, output_dir):
        ...
        try:
            ...
            if 'is_toxic' in df.columns and 'comment.body' in df.columns:
                # Filter for toxic comments
                toxic_comments = df[df['is_toxic'] == True].copy()
                
                if not toxic_comments.empty:
                    # Select relevant columns
                    columns_to_keep = ['created_at', 'actor.login', 'comment.body', 'toxicity_score', 'repo.name']
