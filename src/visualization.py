...
def visualize_data():
    ...
    try:
        ...
        if not os.path.exists("toxicity_scores.csv"):
            logging.error("File 'toxicity_scores.csv' not found in the current directory.")
            return
        if not os.path.exists("productivity.csv"):
            logging.error("File 'productivity.csv' not found in the current directory.")
            return

        # Use Dask to read large CSV files
        toxicity_df = dd.read_csv("toxicity_scores.csv")
        productivity_df = dd.read_csv("productivity.csv")

        # Check if the DataFrames have any rows
        if len(toxicity_df.index) == 0 or len(productivity_df.index) == 0:
        ...
        if "comment_author" in toxicity_df.columns and "author" in productivity_df.columns:
            merged_df = dd.merge(toxicity_df, productivity_df, left_on="comment_author", right_on="author", how="inner")
        else:
            logging.info("Required columns missing for merge. Cannot visualize data.")
            return

        # Compute the merged DataFrame for visualization
